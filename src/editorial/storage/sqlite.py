...
class SQLiteArticleRepository:
    def __init__(self, path: str | Path):
        self.path = Path(path); self._initialise()
    ...
    def list(self, status: EditorialStatus | None = None, limit: int | None = None) -> list[Article]:
        query = "SELECT * FROM articles"; params: list[object] = []
        if status is not None:
            query += " WHERE status = ?"; params.append(status.value)
        query += " ORDER BY published_at DESC, created_at DESC"
        if limit is not None:
            query += " LIMIT ?"; params.append(limit)
        with self._connect() as conn: rows = conn.execute(query, params).fetchall()
        return [self._row_to_article(row) for row in rows]
    def count(self) -> int:
        with self._connect() as conn: row = conn.execute("SELECT COUNT(*) AS n FROM articles").fetchone()
        return int(row["n"])
    def get(self, article_id: UUID) -> Article | None:
        with self._connect() as conn: row = conn.execute("SELECT * FROM articles WHERE id = ?", (str(article_id),)).fetchone()
